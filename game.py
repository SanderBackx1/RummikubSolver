...
class RummikubBoard(_RKB, Node):
    ...
    def reward(board):
        ...
        if board.winner is None:
            return board.turn_moves + 0.5 #should never happen in rummikub
    ...
    def check_if_valid(board, section, block):
        ...
        if section_type == 's':
            ...
            for block in section:
                if block.digit == 0:
                    joker_color = block.color
                
            
            if (digits[0]== block.digit and (block.color not in colors or block.color == joker_color)) or block.digit == 0:
                valid=True
        else:
            # check if same color check if counting 
            if block.digit == 0:
